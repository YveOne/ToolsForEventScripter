...
require(JD_HOME + "/eventscripter/system/overwriteFile.js");
require(JD_HOME + "/eventscripter/system/time.js");
...
var ProxyCleaner = (function () {
    ...
    return function (overwriteCfg) {
        ...
        if (session.inactiveTill >= time(true)) {
            return;
        }
        ...
        if (nextLogTimestamp === lastLogTimestamp) {
            session.inactiveTill = time(true) + cfg.scriptInactiveTime;
            if (session.foundLogs) {
                alert("ProxyCleaner banned " + session.totalBanned + " proxies");
                session.foundLogs = false;
                session.totalBanned = 0;
            }
        } else {
            session.foundLogs = true;
        }
